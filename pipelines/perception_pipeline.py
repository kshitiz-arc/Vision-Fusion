...
class PerceptionPipeline:
    ...
    def __init__(self, cfg: dict) -> None:
        ...
        self.object_detector = ObjectDetector(cfg.get("object_detection", {}))
    ...
    def process_frame(self, raw_frame: np.ndarray) -> PipelineResult:
        ...
        use_yolov8_ids = False
        track_bboxes: list = []
        if self._enabled["track"]:
            if result.object_detections:
                # detect whether YOLOv8 provided its own IDs
                for det in result.object_detections:
        ...
        if self._enabled["track"] and use_yolov8_ids:
            # YOLOv8 already tracked; convert detections to Track objects
            tracks = []
            for det in result.object_detections:
                ...
                tracks.append(Track(track_id=det.track_id,
                                    bbox=det.bbox,
                                    centroid=(cx, cy),
                                    history=[(cx, cy)]))
            logger.debug("YOLOv8 provided %d tracks: %s", len(tracks),
                         [t.track_id for t in tracks])
            working = CentroidTracker.draw(working, tracks)
            result.tracks = tracks
        elif self._enabled["track"] and track_bboxes:
